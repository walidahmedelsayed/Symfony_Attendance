...
class appDevDebugProjectContainerUrlMatcher extends Symfony\Bundle\FrameworkBundle\Routing\RedirectableUrlMatcher
{
    ...
    public function match($pathinfo)
    {
        ...
        if (0 === strpos($pathinfo, '/branches')) {
            // app_branches_get
            if ($pathinfo === '/branches') {
                if (!in_array($this->context->getMethod(), array('GET', 'HEAD'))) {
                    $allow = array_merge($allow, array('GET', 'HEAD'));
            ...
            if (preg_match('#^/branches/(?P<id>[^/]++)$#s', $pathinfo, $matches)) {
                if (!in_array($this->context->getMethod(), array('GET', 'HEAD'))) {
                    $allow = array_merge($allow, array('GET', 'HEAD'));
        ...
        if (0 === strpos($pathinfo, '/r')) {
            if (0 === strpos($pathinfo, '/requests')) {
                ...
                if (preg_match('#^/requests/(?P<id>[^/]++)$#s', $pathinfo, $matches)) {
                    if (!in_array($this->context->getMethod(), array('GET', 'HEAD'))) {
                        $allow = array_merge($allow, array('GET', 'HEAD'));
            ...
            if (0 === strpos($pathinfo, '/rule')) {
                // app_rule_get
                if ($pathinfo === '/rule') {
                    if (!in_array($this->context->getMethod(), array('GET', 'HEAD'))) {
                        $allow = array_merge($allow, array('GET', 'HEAD'));
                        goto not_app_rule_get;
                    }

                    return array (  '_controller' => 'AppBundle\\Controller\\RuleController::getAction',  '_route' => 'app_rule_get',);
                }
                not_app_rule_get:

                // app_rule_getrule
                if (preg_match('#^/rule/(?P<id>[^/]++)$#s', $pathinfo, $matches)) {
                    if (!in_array($this->context->getMethod(), array('GET', 'HEAD'))) {
                        $allow = array_merge($allow, array('GET', 'HEAD'));
                        goto not_app_rule_getrule;
                    }

                    return $this->mergeDefaults(array_replace($matches, array('_route' => 'app_rule_getrule')), array (  '_controller' => 'AppBundle\\Controller\\RuleController::getRuleAction',));
                }
                not_app_rule_getrule:

                // app_rule_update
                if (preg_match('#^/rule/(?P<id>[^/]++)$#s', $pathinfo, $matches)) {
                    if ($this->context->getMethod() != 'PUT') {
                        $allow[] = 'PUT';
                        goto not_app_rule_update;
                    }

                    return $this->mergeDefaults(array_replace($matches, array('_route' => 'app_rule_update')), array (  '_controller' => 'AppBundle\\Controller\\RuleController::updateAction',));
                }
                not_app_rule_update:
        ...
        if (0 === strpos($pathinfo, '/studentsAttendance')) {
            // app_studentsattendance_get
            if ($pathinfo === '/studentsAttendance') {
                if (!in_array($this->context->getMethod(), array('GET', 'HEAD'))) {
                    $allow = array_merge($allow, array('GET', 'HEAD'));
            ...
            if (preg_match('#^/studentsAttendance/(?P<id>[^/]++)$#s', $pathinfo, $matches)) {
                if (!in_array($this->context->getMethod(), array('GET', 'HEAD'))) {
                    $allow = array_merge($allow, array('GET', 'HEAD'));
        ...
        if (0 === strpos($pathinfo, '/tracks')) {
            ...
            // app_tracks_gettrack
            if (preg_match('#^/tracks/(?P<id>[^/]++)$#s', $pathinfo, $matches)) {
                if (!in_array($this->context->getMethod(), array('GET', 'HEAD'))) {
                    $allow = array_merge($allow, array('GET', 'HEAD'));
